...
layout(std430, binding = 2) buffer world_position_buffer {
    vec4 pos[];
};
layout(std430, binding = 3) buffer coeff_buffer {
    vec2 coeff[];
};
layout(std430, binding = 4) buffer adj_face_buffer {
    int adjface[];
};
layout(std430, binding = 5) buffer should_redraw_buffer {
    int should_redraw[];
};
...
bool Iteration(int id, float x0, float y0, inout float u, inout float v, inout float w) {
...
void main() {
    ...
    vec4 input_value = imageLoad(imgInput, texCoord);
	vec4 output_value = imageLoad(imgOutput, texCoord);

    if (input_value.w < 1.01f) {
        return;
    }
    if (output_value.w > 1.01f) {
        return;
	}
    
    //imageStore(imgInput, texCoord, vec4(1.0));

    float x0 = 0.5f + texCoord.x;
    float y0 = 0.5f + texCoord.y;

    float u = input_value.x;
    float v = input_value.y;
    float w = 1.0f - u - v;
    int id = int(round(input_value.w - 2.0f));
    ...
    if (Iteration(id, x0, y0, u, v, w)) {
        normal = computeNormal(id, u, v, w);
        imageStore(imgOutput, texCoord, vec4(normal, id + 2.0f));
        return;
    }
    ...
    for (int i = 0; i < 3; i++) {
        ...
        if (Iteration(adjId, x0, y0, u, v, w)) {
            normal = computeNormal(adjId, u, v, w);
            imageStore(imgOutput, texCoord, vec4(normal, adjId + 2.0f));
            return;
        }
    }
}